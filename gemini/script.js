...
async function sendPrompt() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            displayResponse(story, responseDiv, totalTokens, model, duration, userMessage, story);
...
function calculateCost(model, inputChars, outputChars) {
    let inputCostPerK, outputCostPerK;

    if (model === 'gemini-1.5-flash') {
        inputCostPerK = inputChars > 128000 ? 0.025 : 0.0125;
        outputCostPerK = inputChars > 128000 ? 0.075 : 0.0375;
    } else if (model === 'gemini-1.5-pro') {
        inputCostPerK = inputChars > 128000 ? 0.25 : 0.125;
        outputCostPerK = inputChars > 128000 ? 0.75 : 0.375;
    } else if (model === 'gemini-1.0-pro') {
        inputCostPerK = 0.0125;
        outputCostPerK = 0.0375;
    } else {
...
function displayResponse(content, responseDiv, totalTokens, model, duration, userMessage, story) {
    responseDiv.innerHTML = content;

    const inputChars = userMessage.length;
    const outputChars = story.length;

    const { totalInputCost, totalOutputCost, totalCost } = calculateCost(model, inputChars, outputChars);

    accumulatedCost += totalCost;

    const tokenInfo = document.createElement('div');
    tokenInfo.className = 'token-info';
    tokenInfo.innerText = `Time taken: ${duration} seconds, Tokens in: ${inputChars}, Tokens out: ${outputChars}, Chars in: ${inputChars}, Chars out: ${outputChars},  respose cost: ${(totalCost * 100).toFixed(2)}¢, Accumulated cost: ${(accumulatedCost * 100).toFixed(2)}¢, Model: ${model}`;
